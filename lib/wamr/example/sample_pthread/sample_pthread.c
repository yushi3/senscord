...
static void* thread_func(void* args) {
  int32_t ret = 0;
  senscord_core_t core = *(senscord_core_t*)args;
...
int32_t main(int32_t argc, const char* argv[]) {
  ...
  if (ret != 0) {
    PRINT("senscord_core_init(): cause=%d\n",
          senscord_get_last_error_cause());
    exit(-1);
  }

  for (i = 0; i < THREAD_NUM; ++i) {
    ret = pthread_create(&thread[i], NULL, thread_func, &core);
    if (ret != 0) {
  ...
  if (ret != 0) {
    PRINT("senscord_core_exit(): cause=%d\n",
          senscord_get_last_error_cause());
    exit(-1);
  }
