 ...
 */
Status RegisterAccess32PropertyComponent::MsgPackToBinary(
    std::vector<uint8_t>* src, std::vector<uint8_t>* dst) {

  RegisterAccess32Property property = {};
  ...
  property.element =
      std::vector<RegisterAccessElement<uint32_t> >(property_js.element.size());
  for (size_t i = 0; i < property_js.element.size(); ++i) {
    ...
    property.element[i].data = property_js.element[i].data;
  }
 ...
 * @return Status object.
 */
Status RegisterAccess32PropertyComponent::BinaryToMsgPack(
    std::vector<uint8_t>* src, std::vector<uint8_t>* dst) {
