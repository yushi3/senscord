 ...
 */
Status RegisterAccess08PropertyComponent::MsgPackToBinary(
    std::vector<uint8_t>* src, std::vector<uint8_t>* dst) {

  RegisterAccess8Property property = {};
  ...
  property.element =
      std::vector<RegisterAccessElement<uint8_t> >(property_js.element.size());
  for (size_t i = 0; i <property_js.element.size(); ++i) {
    ...
    if (property_js.element[i].data > std::numeric_limits<uint8_t>::max()) {
    ...
    property.element[i].data =
        static_cast<uint8_t>(property_js.element[i].data);
  }
 ...
 * @return Status object.
 */
Status RegisterAccess08PropertyComponent::BinaryToMsgPack(
    std::vector<uint8_t>* src, std::vector<uint8_t>* dst) {
