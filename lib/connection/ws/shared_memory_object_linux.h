 ...
 */
#ifndef LIB_CONNECTION_WS_SHARED_MEMORY_OBJECT_LINUX_H_
#define LIB_CONNECTION_WS_SHARED_MEMORY_OBJECT_LINUX_H_
...
#define CreateSharedMemoryObject() \
  new senscord::ws::SharedMemoryObjectLinux()
    ...
    if (fd_ != -1) {
      ...
      if (fcntl(fd_, F_SETLK, &fl) == 0) {
        if (shm_unlink(name_.c_str()) != 0) {
          LOG_W("[Shared memory] shm_unlink failed: %s", strerror(errno));
        }
      }
      close(fd_);
      fd_ = -1;
...
#endif  // LIB_CONNECTION_WS_SHARED_MEMORY_OBJECT_LINUX_H_
