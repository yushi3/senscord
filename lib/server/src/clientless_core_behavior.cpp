...
Status ClientlessCoreBehavior::OpenStream(
  ...
  const StreamSetting* stream_config =
    config_manager->GetStreamConfigByStreamKey(key);
  ...
  StreamSetting open_config = *stream_config;
  if (setting != NULL) {
    open_config.frame_buffering = setting->frame_buffering;
  }

  // set the real address. (clientless)
  open_config.address = open_config.radical_address;

  Status status = config_manager->VerifyStreamConfig(&open_config);
  if (!status.ok()) {
    return SENSCORD_STATUS_TRACE(status);
  }
